...
int main(int argc, char *argv[])
{
	...
	fprintf(stderr, "sumstr=%s", sr_hash2sumstr(wsumstr,sr_sumstr2hash(sumhash,"z,d")));
	...
	for (i = 0; i < 9; i++) {
		...
		memcpy(rt_hash, sr_sumstr2hash(sumhash, osumstr), SHA512_DIGEST_LENGTH + 1);
		...
		memcpy(original_hash, sr_sumstr2hash(sumhash, rtsumstr), SHA512_DIGEST_LENGTH + 1);
	}
	t = time(NULL);
	sr_log_msg(NULL,LOG_INFO, "              It is now: %s\n", ctime(&t));

	/* FIXME, repeat for MD5, for N, n, L, R  */

	clock_gettime(CLOCK_REALTIME, &tsnow);

	sr_log_msg(NULL,LOG_INFO, "        starting time is: %ld\n", tsnow.tv_sec);
	tscp = sr_time2str(&tsnow);
	memset(timestring, 0, 60);
	memcpy(timestring, tscp, strlen(tscp));
	sr_log_msg(NULL,LOG_DEBUG, "text version ot time is: %s\n", timestring);

	memcpy(&tsrt, sr_str2time(timestring), sizeof(struct timespec));
	sr_log_msg(NULL,LOG_DEBUG, " round tripped, time is: %ld\n", tsrt.tv_sec);
	sr_log_msg(NULL,LOG_DEBUG, "          difference is: %ld\n", tsrt.tv_sec - tsnow.tv_sec);
	...
	sr_log_msg(NULL,LOG_INFO, " testing sr_hex2base64 conversion\n");
